...
public class SelfNum_4673 {
	...
	public static void main(String[] args) {
		/*
		 * 문제 셀프 넘버는 1949년 인도 수학자 D.R. Kaprekar가 이름 붙였다. 양의 정수 n에 대해서 d(n)을 n과 n의 각 자리수를
		...
		for(int i=1; i<=10000; i++) {
			...
			if(n<=10000) {
				check[n] = true; // 셀프 넘버가 맞으면 true로 바꿔줌
			}
		}
		
		for(int i = 1; i<=10000; i++) {
			if(!check[i]) { // check[i] == false인 것만 출력해야하므로
				System.out.println(i);
			}
		}
	}

	public static int selfNum(int n) {// 생성자가 있는 수를 return
		int sum = n;
